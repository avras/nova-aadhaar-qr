...
pub(crate) struct RSASigHasher<Scalar: PrimeFieldBits> {
    constants: PoseidonConstants<Scalar, typenum::U4>,
    io_pattern: IOPattern,
}

impl<Scalar: PrimeFieldBits> RSASigHasher<Scalar> {
    pub(crate) fn new(num_absorbs: u32) -> Self {
        Self {
...
pub struct AadhaarIOHasher<Scalar: PrimeFieldBits> {
    constants: PoseidonConstants<Scalar, typenum::U8>,
    io_pattern: IOPattern,
}

impl<Scalar: PrimeFieldBits> AadhaarIOHasher<Scalar> {
    pub fn new(num_absorbs: u32) -> Self {
        Self {
            constants: Sponge::<Scalar, typenum::U8>::api_constants(neptune::Strength::Standard),
            io_pattern: IOPattern(vec![SpongeOp::Absorb(num_absorbs), SpongeOp::Squeeze(1)]),
        }
    }
